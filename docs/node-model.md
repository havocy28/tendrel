...
One file per node, `graph/<ID>.md`: YAML frontmatter + a lab-notebook body.
...
| `observation` | `OBS-` | A pattern/anomaly noticed; no lifecycle | — |

Per-kind attributes (expected, not enforced): `experiment` → `question`, `config`; `theory` →
`confidence` (low/moderate/high), `next_gate`; `pipeline_node` → optional `eval`.
