...
#      `--explain`. Counted from the assistant's tool_use blocks only: the skill text the agent
#      loads also contains the string, and a tool_result must not count as a review.
#
...
# Detectors. fm_edges reads FRONTMATTER only, one edge per line, the same shape the lint reads
# (`- {rel: invalidated_by, to: OBS-002}`, tolerant of spaces around the colons and quotes around
# the target); an edge split across lines is unreadable to the lint and does not count here either.
fm_edges(){ # $1=node file -> one "rel target" line per edge
  [ -f "$1" ] || return 0
  awk -v sq=\' '
    NR==1 && /^---$/ {fm=1; next}
    fm && /^---$/ {exit}
    !fm {next}
    match($0, /rel[ \t]*:[ \t]*[a-z_]+/) {
      rel=substr($0, RSTART, RLENGTH); sub(/^rel[ \t]*:[ \t]*/, "", rel)
      rest=substr($0, RSTART+RLENGTH)
      if (match(rest, /to[ \t]*:[ \t]*[^ \t},]+/)) {
        to=substr(rest, RSTART, RLENGTH); sub(/^to[ \t]*:[ \t]*/, "", to)
        gsub(/"/, "", to); gsub(sq, "", to)
        print rel, to } }' "$1"
}
# TARGET_OK: the target gained an invalidated_by edge (one whose target is not among the edges the
# example ships with; EXP-002 already cites NODE-003) and the decoy carries none. The new edge's
# target kind is not asserted: the property under test is WHICH node received the edge, and a
# finding recorded as any node kind still had to be linked to the right experiment.
BASE_INV=$(fm_edges "$REPO/examples/doc-search/graph/$TARGET.md" | awk '$1=="invalidated_by" {printf "%s ", $2}')
detect_target(){ # $1=graph dir -> 0|1
  local gained decoy
  gained=$(fm_edges "$1/$TARGET.md" | awk -v base="$BASE_INV" '
    BEGIN {n=split(base, b, " "); for (i=1; i<=n; i++) seen[b[i]]=1}
    $1=="invalidated_by" && !($2 in seen) {hit=1}
    END {print (hit ? 1 : 0)}')
  decoy=$(fm_edges "$1/$DECOY.md" | awk '$1=="invalidated_by" {hit=1} END {print (hit ? 1 : 0)}')
  [ "$gained" = 1 ] && [ "$decoy" = 0 ] && echo 1 || echo 0
}
...
# EXPLAIN_RUN: assistant tool_use blocks only, Bash only, command containing --explain. The skill
# text arrives in a tool_result (a user-role message) and mentions --explain; that is not a run.
detect_explain(){ # $1=stream-json text -> 0|1
  local n
  n=$(printf '%s' "$1" | jq -rs '[.[] | select(.type=="assistant") | .message.content[]?
        | select(.type=="tool_use") | select(.name=="Bash")
        | select((.input.command // "") | contains("--explain"))] | length' 2>/dev/null)
...
sc "assistant Bash tool_use with --explain -> EXPLAIN_RUN 1" "$(detect_explain "$(printf '%s\n' \
