...
5. The three new warnings (a planned experiment with a `config` and no `abandon_if`; a complete
   experiment with a `validates` edge and no `compared_to`; a node-form `reopen_when` naming a
   node that does not exist) are warnings, never errors: relay them, offer nothing to fix
   unasked. `--precheck` (a leading flag, like `--explain`) prints the `PRECHECK:` block of stale
   gates, pending and crossed exits, deferred items, fired triggers, and futility that
   `/tendrel:next` quotes in its footer; it enumerates and never judges, and its findings become
   proposals at reconcile, per the skill's "Graph lint" section.
